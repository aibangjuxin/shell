## 0.1. References
* https://raw.githubusercontent.com/pydevops/gcloud-cheat-sheet/master/gcloud.md
...
gcloud container clusters describe $cluster_name --region=$region --format='flattened(currentNodeVersion,name,nodeConfig.diskSizeGb,nodeConfig.serviceAccount,nodePools[0].name,nodeConfig.tags,nodeConfig.bootDiskKmsKey,location)'

gcloud container node-pools create $node_pools_name-1 \
--boot-disk-kms-key=$kms \
--cluster=$cluster_name --disk-size=100G --disk-type=pd-standard --machine-type=n1-standard-8 \
--node-locations=$location-a,$location-b,$location-c \
--node-version=$gkeversion \
--tags=$firewall \
--enable-autoscaling \
--num-nodes=1 --max-nodes=15 --min-nodes=2 \
--region=$region \
...
```bash
gcloud container images 1ist
gcloud beta container images describe "$fully_qualified_digest" --show-package-vulnerability --format=json|jq '.discovery_summary.discovery[0].discovery,.discovery_summary.discovery[0].updateTime
...
bucket_prefix=$(gcloud projects describe $project --format="value(labels.bucket_prefix)")
...
gcloud compute instances list --filter="name~'^keyword*' OR name~'^key*'"
gcloud compute instances describe mig-name --zone europe-west2-c --format="value(labels.mig_name)"
#list firewall tags
gcloud compute instances describe $vpc_instance_name --zone $vpc_instance_zone --format='value(tags.items)'
